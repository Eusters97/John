...
export default function EnhancedPaymentModal() {
  ...
  const [paymentStep, setPaymentStep] = useState<PaymentStep>({ step: 'select' });
  ...
  const handleCryptoSelect = async (cryptoSymbol: string) => {
    ...
    try {
      ...
      setPaymentStep({ 
        step: 'error', 
        data: { message: error instanceof Error ? error.message : 'Failed to create payment' }
      });
      toast({
        title: "Payment Error",
  ...
  const checkPaymentStatus = async () => {
    ...
    try {
      const status = await nowPaymentsService.updatePaymentStatus(paymentData.payment.payment_id);
      ...
      if (status.payment_status === 'finished') {
        ...
        setPaymentStep({ 
          step: 'error', 
          data: { message: `Payment ${status.payment_status}. Please try again.` }
        });
      }
    } catch (error) {
  ...
  const getPaymentStatusBadge = (status: string) => {
    const statusConfig = {
      waiting: { color: 'bg-yellow-500', text: 'Waiting for Payment', icon: Clock },
      ...
      partially_paid: { color: 'bg-orange-500', text: 'Partially Paid', icon: AlertCircle },
  ...
  const selectedCryptoData = cryptoCurrencies.find(crypto => crypto.currency === selectedCrypto);
  ...
  return (
    <Dialog open={showPaymentModal} onOpenChange={setShowPaymentModal}>
      <DialogContent className="max-w-4xl p-0 bg-gradient-to-br from-gray-50 via-white to-forex-50 border-0 overflow-hidden">
        <DialogTitle className="sr-only">Enhanced Investment Payment Process</DialogTitle>
        ...
        <div className="p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Secure cryptocurrency payment processing powered by NOWPayments</p>
          </div>
          ...
          <div className="flex justify-center mb-8">
            <div className="flex space-x-4">
              <div className={`flex items-center space-x-2 ${paymentStep.step === 'select' ? 'text-forex-600' : ['payment', 'confirmation'].includes(paymentStep.step) ? 'text-success-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${paymentStep.step === 'select' ? 'bg-forex-600 text-white' : ['payment', 'confirmation'].includes(paymentStep.step) ? 'bg-success-600 text-white' : 'bg-gray-200'}`}>
                  {['payment', 'confirmation'].includes(paymentStep.step) ? <CheckCircle className="h-4 w-4" /> : '1'}
                </div>
                <span className="font-medium">Select Crypto</span>
              </div>
              <div className={`flex items-center space-x-2 ${paymentStep.step === 'payment' ? 'text-forex-600' : paymentStep.step === 'confirmation' ? 'text-success-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${paymentStep.step === 'payment' ? 'bg-forex-600 text-white' : paymentStep.step === 'confirmation' ? 'bg-success-600 text-white' : 'bg-gray-200'}`}>
                  {paymentStep.step === 'confirmation' ? <CheckCircle className="h-4 w-4" /> : paymentStep.step === 'payment' ? <Clock className="h-4 w-4" /> : '2'}
                </div>
                <span className="font-medium">Payment</span>
              </div>
              <div className={`flex items-center space-x-2 ${paymentStep.step === 'confirmation' ? 'text-success-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${paymentStep.step === 'confirmation' ? 'bg-success-600 text-white' : 'bg-gray-200'}`}>
                  {paymentStep.step === 'confirmation' ? <CheckCircle className="h-4 w-4" /> : '3'}
                </div>
          ...
          {paymentStep.step === 'select' && (
            <div className="space-y-6">
              <h3 className="text-xl font-semibold text-center text-gray-900">Choose Your Cryptocurrency</h3>
              {loading ? (
                  ...
                  {cryptoCurrencies.map(crypto => (
                    ...
                    >
                      <CardContent className="p-4 text-center">
                        ...
                        />
                        <h4 className="font-semibold text-sm mb-1">{crypto.name}</h4>
                        <p className="text-xs text-gray-600 uppercase">{crypto.currency}</p>
          ...
          {paymentStep.step === 'payment' && paymentData && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-600">Send exactly <strong>{paymentData.payment.pay_amount} {paymentData.payment.pay_currency.toUpperCase()}</strong> to complete your investment</p>
                {timeLeft > 0 && (
                  <div className="mt-2 flex items-center justify-center space-x-2">
                    <Clock className="h-4 w-4 text-orange-500" />
                    <span className="text-orange-600 font-mono">{formatTime(timeLeft)} remaining</span>
                  </div>
                )}
              </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span>{paymentData.payment.pay_currency.toUpperCase()} Payment</span>
                      </div>
                  ...
                  <CardContent className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => copyToClipboard(paymentData.payment.pay_address, 'Payment address')}
                        >
                    ...
                    <div className="bg-forex-50 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 mb-2">Exact Amount to Send:</div>
                      <div className="text-2xl font-bold text-forex-600">
                        {paymentData.payment.pay_amount} {paymentData.payment.pay_currency.toUpperCase()}
                      </div>
                      <div className="text-sm text-gray-500">≈ ${paymentData.payment.price_amount} USD</div>
                    </div>

                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 mb-2">Payment ID:</div>
                      <div className="font-mono text-sm">{paymentData.payment.payment_id}</div>
                    </div>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-3">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <div className="font-medium">Copy the payment address</div>
                          <div className="text-sm text-gray-600">Use the copy button to avoid mistakes</div>
                        </div>
                      </div>
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600">{paymentData.payment.pay_amount} {paymentData.payment.pay_currency.toUpperCase()}</div>
                        </div>
                      </div>
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-600">Payment will be automatically verified</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                      ...
                      <div className="text-sm text-yellow-700 mt-1">
                        Send only {paymentData.payment.pay_currency.toUpperCase()} to this address. 
                        Sending other cryptocurrencies will result in loss of funds.
                      </div>
                    </div>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      ...
                      <div className="text-sm text-blue-700 mt-1">
                        Payments typically confirm within 10-30 minutes. 
                        You'll be notified once your investment is activated.
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {paymentStep.step === 'confirmation' && (
            <div className="text-center space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-2">Investment Activated!</h3>
                <p className="text-gray-600 max-w-md mx-auto">
                  Your payment has been confirmed and your investment is now active. 
          ...
          {paymentStep.step === 'error' && (
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600 max-w-md mx-auto">
                  {paymentStep.data?.message || 'Something went wrong with your payment. Please try again.'}
                </p>
              </div>
