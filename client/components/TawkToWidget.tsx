  ...
  useEffect(() => {
        ...
        script.onload = () => {
          ...
          if (window.Tawk_API) {
            window.Tawk_API.onLoad = function() {
              ...
              setTimeout(() => {
                const tawkFrame = document.querySelector('#tawk-to-widget, [id^="tawk"], iframe[src*="tawk.to"]');
                if (tawkFrame) {
    ...
    const retryTimer = setTimeout(() => {
      if (!loaded && attempts < 3) {
        console.log(`💬 Retrying Tawk.to initialization (attempt ${attempts + 1})`);
        initTawkTo();
      }
    }, 5000);
    ...
    if (loaded) {
      const checkVisibility = () => {
        ...
        if (tawkElements.length === 0) {
          console.warn('⚠️ No Tawk.to elements found - widget may not be visible');
        } else {
